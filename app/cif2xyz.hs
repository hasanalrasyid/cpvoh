...
import qualified CIF.Main as C
...
main = do
  ...
  let inFile = _inFile opts
  s <- readProcess "cif2cell" [inFile] []
  putStrLn $ show $ head $ tail $ splitWhen (isPrefixOf "Atom") $ lines s
--  f <- getContents
--  cif <- C.parseCIFString f
--  putStrLn $ show cif
--    processZMatrix opts
...
optsParser = Opts
             ...
             <*> strOption (long "in" <> short 'i' <> metavar "IN_CIF"
                            <> help "CIF input file")
